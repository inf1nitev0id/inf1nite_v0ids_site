...
Vue.component('file-input', {
  props: ['name', 'btnClass', 'maxCount', 'maxSize', 'maxFileSize', 'canChangeName', 'accept'],
  data: function data() {
    return {
      files: []
    };
  ...
  methods: {
    addFile: function addFile() {
      ...
      setTimeout(function () {
        _this.$refs['input' + (_this.files.length - 1)][0].click();
      }, 10);
    },
    ...
    selectFile: function selectFile(index) {
      var files = this.$refs['input' + index][0].files;

      if (files.length) {
        this.files[index].file = files[0];
        this.files[index].name = files[0].name;
        this.files[index].icon = files[0].type.match(/^image\//) ? URL.createObjectURL(files[0]) : 'far fa-file';
      } else {
        this.files.splice(index, 1);
      }
    },
    changeName: function changeName(index) {
      ...
      var extension = file.file.name.match(/^.+(\..+?)$/)[1].toLowerCase();
