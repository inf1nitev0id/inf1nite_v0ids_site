        ...
        <form class="col-xs-12 col-md-6" method="post" action="{{ route('mahouka.top.load-hashes') }}">
            ...
            <table>
                ...
                @foreach ($unknown_names as $hash => $name)
                    <tr>
                        <td>
                            <svg vievBox="0 0 {{ count($name) }} {{ $char_height }}" width="{{ count($name) / 2 }}"
                                 height="{{ $char_height / 2 }}" xmlns="http://www.w3.org/2000/svg">
                                @foreach ($name as $x => $line)
                                    @foreach ($line as $y => $pixel)
                                        @if ($pixel)
                                            <rect fill="#000" x="{{ $x / 2 }}" y="{{ $y / 2 }}" width="0.5"
                                                  height="0.5"/>
                                        @endif
                                    @endforeach
                                @endforeach
                ...
                @foreach ($unknown_numbers as $hash => $number)
                    <tr>
                        <td>
                            <svg vievBox="0 0 {{ count($number) }} {{ $char_height }}" width="{{ count($number) }}"
                                 height="{{ $char_height }}" xmlns="http://www.w3.org/2000/svg">
