    ...
    <div id="chart">
        ...
        <div v-cloak style="position: relative;">
            ...
            <div v-for="y in horizontalDivisions" class="axis-text"
                 :style="{position: 'absolute', top: y.y + 5 + 'px', left: '5px'}">
                @{{ y.value }}
            </div>
                ...
                >
                    <g transform="translate(0, 5)">
                        <g>
                            ...
                            <template v-for="day in eventsDays">
                                <template v-for="(event, index) in day.events">
                                    <text @click="selected_event = event; selected_event.date = day.date" class="event"
                                          :class="{important: event.important}" :fill="event.color" :x="day.x"
                                          :y="11 + index * 10" text-anchor="middle">
                                        @{{ event.type }}
                                    </text>
                                    <line class="event" :class="{important: event.important}" :stroke="event.color"
                                          :x1="day.x" :y1="day.events.length * 10 + 4" :x2="day.x" :y2="sizeY"/>
                                </template>
                            </template>
                        </g>
                        <g class="chart-grid">
                            ...
                            <text v-for="(date, index) in dates" class="axis-text" :x="index * dayWidth + dayWidth / 2"
                                  :y="sizeY + 13" text-anchor="middle">
                                @{{ date }}
                            </text>
                            <text v-for="month in months" class="axis-text" :x="month.x" :y="sizeY + 28"
                                  text-anchor="middle">
                                @{{ month.text }}
                            </text>
                        </g>
                        <g :transform="'translate(0,' + sizeY + ')'">
                            {{-- графики --}}
                            <polyline v-for="line in lines" class="line" v-show="line.visible"
                                      @click.left="setSelected(line.user.id)"
                                      @click.right.prevent="line.visible = false" :stroke="line.color"
                                      :points="points[line.index]"/>
                        </g>
                        <g v-if="selected != 0 && selectedLine.visible" :transform="'translate(0,' + sizeY + ')'">
                            {{-- выбранный график --}}
                            <polyline class="line selected" @click.left="setSelected(selectedLine.user.id)"
                                      @click.right.prevent="selectedLine.visible = false" :stroke="selectedLine.color"
                                      :points="points[selectedLine.index]"/>
                            <template
                                v-for="point in chart[selectedLine.index].filter(function(point) { return point.rate !== null })">
                                <text v-for="style in ['rate-text-border', 'rate-text']" :class="style"
                                      :x="isUp(point.y, point.rate) ? -5 : 5" y="2"
                                      :text-anchor="isUp(point.y, point.rate) ? 'end' : 'start'"
        ...
        <div v-cloak class="form-inline">
            ...
            <div class="btn-group m-1">
                <div class="btn btn-outline-secondary icon-btn" :class="{active: changes_mode}" title="Режим изменений"
                     @click="changes_mode = !changes_mode">
                    <i class="fas fa-chart-line"></i>
                </div>
                <div v-if="changes_mode" class="btn btn-outline-secondary icon-btn" :class="{active: day_mode}"
                     title="Суммировать значения за день" @click="day_mode = !day_mode">
                    <i class="fas fa-calendar-day"></i>
                </div>
            </div>
            <div class="btn-group m-1">
                @if (Auth::check() && Auth::user()->role === 'admin')
                    <a class="btn btn-outline-secondary icon-btn" href="{{ route('mahouka.top.edit') }}"><i
        ...
        <div v-cloak class="tab-content">
            <div class="tab-pane fade show active" id="users">
                ...
                <div v-for="line in lines" class="btn-group m-1">
                    <div class="btn option-btn"
                         :class="'btn-outline-' + (selected == line.user.id ? 'secondary' : 'light')"
                         :style="{color: line.color}" @click="setSelected(line.user.id)">
                        @{{ line.user.name }}
                    </div>
                    <div class="btn" :class="'btn-outline-' + (selected == line.user.id ? 'secondary' : 'light')"
                         :style="{color: line.color}" @click="line.visible = !line.visible">
            ...
            <div class="tab-pane fade" id="events">
                ...
                <div v-for="serie in series" class="option" :style="{color: serie.color}"
                     @click="serie.visible = !serie.visible">
                    <i v-if="serie.visible" class="far fa-check-square"></i>
                    <i v-else class="far fa-square"></i>
                    @{{ serie.name }}
                </div>
                <p class="mt-2 mb-1">Категории:</p>
                <div v-for="type in types" class="option" :style="{color: type.color}"
            ...
            <div class="tab-pane fade form-inline pt-2" id="range">
                ...
                <select v-model="start_indent" class="form-control">
                    <option v-for="date in dates_full.filter(item => item.id < days_full - end_indent)"
                            :value="date.id">@{{ date.full }}
                    </option>
                </select>
                до
                <select v-model="end_indent" class="form-control">
                    <option v-for="date in dates_full.filter(item => item.id >= start_indent)"
                            :value="days_full - date.id - 1">@{{ date.full }}
                    </option>
                </select>
